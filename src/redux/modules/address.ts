...
function* handleFetchAddress() {
  while (true) {
    const { payload }: ReturnType<typeof fetchAddressesRequest> = yield take(
      fetchAddressesRequest.type,
    );
    try {
...
function* handleCreateAddress() {
  while (true) {
    const { payload }: ReturnType<typeof createAddressRequest> = yield take(
      createAddressRequest.type,
    );
    try {

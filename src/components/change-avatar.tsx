...
const ChangeAvatar = () => {
  ...
  const images: string[] = useSelector(
  ...
  const changeAvatar = (image: string) => {
    const changeAvatarRequest: UserUpdateAvatarRequestDTO =
      {} as UserUpdateAvatarRequestDTO;

    changeAvatarRequest.avatar = image;

    if (refreshToken) {
      apiWithToken(refreshToken)
        .patch("/users/update-avatar", changeAvatarRequest, {
          headers: {
  ...
  return (
    <div>
      <div className="p-1">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-2 overflow-scroll scrollbar-rounded max-h-96">
          <div>
            <Button
          ...
          {images.map((url, index) => (
            <div key={index} className="relative inline-block">
              <div onClick={() => handleToggleFileSelect(url)}>
                <img
                  src={url}
                  alt={`Image ${index}`}

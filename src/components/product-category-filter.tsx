...
const ProductCategoryFilter = ({
  ...
  categoryList: ProductCategoryAPIResponse[];
}) => {
  ...
  const getAllCategories = (): void => {
    const items: ICategory[] = categoryList
      .map((category: ProductCategoryAPIResponse) =>
  ...
  const convertCategory = (
    ...
    const result: ICategory[] = [
      {
        ...
        label: (
          <span className="flex items-center">
            ...
            <img
              src={server + category.thumbnail}
              className="w-4 h-4 mr-2"
              alt={category.name}
            />
            <span
              className={`${category.parentProductCategory === null ? "font-bold uppercase" : ""}`}
            >
              {category.name}
            </span>
          </span>
    ...
    if (category.productCategories?.length) {
      category.productCategories.forEach(
        (subCategory: ProductCategoryAPIResponse): void => {
  ...
  return (
    <div className="p-4 bg-white rounded-lg">
      ...
      <div className="mb-4">
        <Checkbox onChange={onInStockChange}>

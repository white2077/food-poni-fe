...
import { fetchProductAction, updateProductDetailSelected } from "@/redux/modules/product.ts";
...
export default function ProductWrapper() {
    ...
    const { product, productDetails } = useSelector((state: RootState) => state.product.productSelected);

    const { productDetailSelected } = useSelector((state: RootState) => state.product);
    ...
    if (!product || !productDetails || productDetails.length < 1 || !productDetailSelected) {
        ...
        <div className='grid gap-4'>
            <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[2fr_3fr_2fr] gap-4'>
                <div className="lg:sticky top-5">
                    <ProductGallery images={Array.from(new Set(productDetailSelected.images))} />
                </div>
                <div className='grid gap-4 lg:order-1 order-2'>
                    <Card size='small'>
                        <h2 className='text-xl'>{product.name + " - " + productDetailSelected.name}</h2>
                        <div className="my-2 flex flex-wrap items-center">
                            ...
                            <span className="border-r-2 py-1 px-1 hidden md:inline">
                                <span
                                    className="text-lg m-1 border-b-2">{productDetailSelected.rateCount}</span> Đánh giá
                            </span>
                            <span className="border-r-2 py-1 px-1">
                                <span
                                    className="text-lg m-1 border-b-2">{productDetailSelected.sales}</span> Lượt bán
                            </span>
                    ...
                    <Card hidden={productDetails.length == 1} size='small' title='Loại sản phẩm'
                        className="static">
                        {(productDetails && productDetails.length > 1) && (
                            <Radio.Group defaultValue={productDetails[0].name || "default"}>
                                {(productDetails || []).map((productDetail) => (
                                    <Radio.Button key={productDetail.id} value={productDetail.name || "default"}
                                        onClick={() => dispatch(updateProductDetailSelected(productDetail))}
            ...
            <ProductRate productDetailSelected={productDetailSelected} />
        </div>
    );
}
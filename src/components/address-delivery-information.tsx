...
export const AddressDeliveryInformation = ({
  ...
  deliveryInformation: AddressAPIResponse[];
}) => {
  ...
  const updateShippingAddress = (addressId: string): void => {
    ...
    if (refreshToken) {
      apiWithToken(refreshToken)
        .patch("/users/update-address", addressIdDTO, {
          headers: {
            Authorization: "Bearer " + accessToken,
          },
        })
  ...
  return (
    <>
      ...
      <div className="w-[1000px] mx-auto font-sans">
        <Button className="my-[16px]" onClick={handleAddAddressClick}>
          {showAddAddress ? "Quay lại" : "Thêm địa chỉ"}
        </Button>
        {showAddAddress && (
          <div className="w-[600px] mx-auto">
            <AddressDeliveryInformationAdd />
          </div>
        )}
        {!showAddAddress && (
          <List
              ...
              <List.Item>
                <Card>
                  <div className="flex justify-between">
                    <div>
                      <div>
                        <span className="nunito mr-[8px] text-orange-500">
                          {item.fullName}
                        </span>
                        <span className="font-sans mr-[8px]">|</span>
                        <span className="font-sans mr-[8px]">
                          {item.phoneNumber}
                        </span>
                    ...
                    <div>
                      <Button
                        type="text"
                        className="!text-purple-600 font-sans"
                      ...
                      <span
                        className="ml-[16px]"
                        hidden={item.id === currentUser.addressId}
                      >
                        <DeleteOutlined
                          onClick={() =>
                            deleteDeliveryInformation(item.id ?? "")
                          }
                        />
                      </span>
                    </div>

...
const RelatedProducts = ({title,query}: ProductRowProps) => {
    ...
    return (
        <div className="p-4 bg-white rounded-lg">
            ...
            <div
                className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 mt-2'>
                {isLoading ? <ProductRowLoading
                    count={8}/> : (productCards.length !== 0 ? productCards.map((productCard: IProductCard) =>
                    <ProductCardRelated key={productCard.id} product={productCard}/>) : "Không có dữ liệu!")}
            </div>
        </div>
